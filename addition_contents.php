...
function loadAdditionContentsEditableV1($contents) {
    $content = $contents[0];
    $subside = $contents[1];
    ...
    if (wCMS::$loggedIn) {
        if ( ! is_null(wCMS::p('delac'))) {
            ...
            if (isset($db->pages->$page->$key)) {
                ...
                for ($i=$k+1 ;$i!=0; $i++) {
                    $addition_content = getContentV1('addition_content_'.$i);
                    $addition_content_show = (getContentV1('addition_content_show_'.$i)=='hide') ? 'hide':'show';
        ...
        if ( ! is_null(wCMS::p('addac'))) {
            $key = wCMS::p('addac');
            $content = wCMS::p('content');
            $db = json_decode(wCMS::db());
            $db->pages->$page->$key = $content;
            list($_, $k) = explode('content_', $key);
            $bf_addition_content = getContentV1('addition_content_'.$k);
            $bf_addition_content_show = (getContentV1('addition_content_show_'.$k)=='hide') ? 'hide':'show';
        ...
        for ($i=1; $i!=0; $i++) {
            ...
            $content.='<p></p>';
            $addition_content_show = getContentV1('addition_content_show_'.$i);
...
function getContentV1($key, $page = false) {
